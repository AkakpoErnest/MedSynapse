...
async function recoverPublicKey({ hash, signature }) {
  ...
  const { secp256k1: secp256k12 } = await import("./secp256k1-ZXWD5PVZ.js");
  const signature_ = (() => {
...
function getCreateAddress(opts) {
  const from8 = toBytes(getAddress(opts.from));
  let nonce = toBytes(opts.nonce);
  if (nonce[0] === 0)
    nonce = new Uint8Array([]);
  return getAddress(`0x${keccak256(toRlp([from8, nonce], "bytes")).slice(26)}`);
}
function getCreate2Address(opts) {
  ...
  return getAddress(slice(keccak256(concat([toBytes("0xff"), from8, salt, bytecodeHash])), 12));
}
...
async function getWebSocketRpcClient(url, options = {}) {
  ...
  return getSocketRpcClient({
    async getSocket({ onClose, onError, onOpen, onResponse }) {
      const WebSocket = await import("./native-PE6EKZWV.js").then((module) => module.WebSocket);
...
var suffixParameters = from("(uint256 chainId, address delegation, uint256 nonce, uint8 yParity, uint256 r, uint256 s), address to, bytes data");
...
function wrap(value) {
  ...
  const self = recoverAddress({
    ...
    signature: from2(value.authorization)
  });
...
async function recoverMessageAddress({ message, signature }) {
  return recoverAddress2({ hash: hashMessage(message), signature });
}
...
async function verifyHash({ address, hash, signature }) {
  return isAddressEqual(getAddress(address), await recoverAddress2({ hash, signature }));
}
...
async function simulateCalls(client, parameters) {
  ...
  const getBalanceData = account ? encode2(from4("constructor(bytes, bytes)"), {
...
function unwrap2(wrapped) {
  assert3(wrapped);
  const [to, data, signature] = decode(from("address, bytes, bytes"), wrapped);
...
async function verifyHash2(client, parameters) {
  ...
  try {
    ...
    try {
      const verified = isAddressEqual(getAddress(address), await recoverAddress2({ hash, signature }));
      if (verified)
        return true;
    } catch {
    }
...
async function sendUnsignedTransaction(client, args) {
  var _a, _b, _c;
  const { accessList, data, from: from8, gas, gasPrice, maxFeePerGas, maxPriorityFeePerGas, nonce, to, value, ...rest } = args;
  const chainFormat = (_c = (_b = (_a = client.chain) == null ? void 0 : _a.formatters) == null ? void 0 : _b.transactionRequest) == null ? void 0 : _c.format;
  const format = chainFormat || formatTransactionRequest;
  const request = format({
    // Pick out extra data that might exist on the chain's transaction request type.
...
//# sourceMappingURL=chunk-LWG3FI4A.js.map
