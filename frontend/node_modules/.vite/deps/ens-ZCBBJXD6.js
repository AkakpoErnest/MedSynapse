...
import {
  ...
  validate2,
  validateObject
} from "./chunk-P53MRDKN.js";
import {
  formatAbi,
  formatAbiItem2 as formatAbiItem,
  parseAbi
} from "./chunk-OBV3X6Y3.js";
...
function bls(CURVE) {
  ...
  function getPublicKey6(privateKey) {
...
var bls12_381 = bls({
  ...
  G2: {
    ...
    fromBytes: (bytes) => {
      ...
      const L = Fp.BYTES;
      const slc = (b, from24, to2) => bytesToNumberBE(b.slice(from24, to2));
...
function fromCreate(options) {
  const from24 = fromHex(from4(options.from));
  let nonce = fromNumber(options.nonce);
  if (nonce[0] === 0)
    nonce = new Uint8Array([]);
  return from4(`0x${keccak256(fromBytes2([from24, nonce], { as: "Hex" })).slice(26)}`);
}
function fromCreate2(options) {
  ...
  return from4(slice2(keccak256(concat(fromHex("0xff"), from24, salt, bytecodeHash), { as: "Hex" }), 12));
}
...
function twistedEdwards(curveDef) {
  ...
  function getPublicKey6(privKey) {
  ...
  function sign6(msg, privKey, options = {}) {
  ...
  function verify6(sig, msg, publicKey, options = verifyOpts) {
...
var hmac2 = (hash7, key, message) => new HMAC(hash7, key).update(message).digest();
hmac2.create = (hash7, key) => new HMAC(hash7, key);
...
function validateSigFormat(format6) {
  if (!["compact", "recovered", "der"].includes(format6))
...
function ecdh(Point2, ecdhOpts = {}) {
  ...
  function keygen(seed) {
    ...
    return { secretKey, publicKey: getPublicKey6(secretKey) };
  }
  ...
  function getSharedSecret4(secretKeyA, publicKeyB, isCompressed = true) {
  ...
  return Object.freeze({ getPublicKey: getPublicKey6, getSharedSecret: getSharedSecret4, keygen, Point: Point2, utils: utils2, lengths });
}
function ecdsa(Point2, hash7, ecdsaOpts = {}) {
  ahash2(hash7);
  ...
  const hmac4 = ecdsaOpts.hmac || ((key, ...msgs) => hmac2(hash7, key, concatBytes3(...msgs)));
  const { Fp: Fp4, Fn } = Point2;
  const { ORDER: CURVE_ORDER, BITS: fnBits } = Fn;
  const { keygen, getPublicKey: getPublicKey6, getSharedSecret: getSharedSecret4, utils: utils2, lengths } = ecdh(Point2, ecdsaOpts);
  ...
  function validateSigLength(bytes, format6) {
    validateSigFormat(format6);
    const size4 = lengths.signature;
    const sizer = format6 === "compact" ? size4 : format6 === "recovered" ? size4 + 1 : void 0;
    return _abytes2(bytes, sizer, `${format6} signature`);
  }
  class Signature {
    ...
    static fromBytes(bytes, format6 = defaultSigOpts_format) {
      validateSigLength(bytes, format6);
      let recid;
      if (format6 === "der") {
        const { r: r2, s: s2 } = DER.toSig(_abytes2(bytes));
        return new Signature(r2, s2);
      }
      if (format6 === "recovered") {
        recid = bytes[0];
        format6 = "compact";
    ...
    static fromHex(hex2, format6) {
      return this.fromBytes(hexToBytes(hex2), format6);
    }
    ...
    toBytes(format6 = defaultSigOpts_format) {
      validateSigFormat(format6);
      if (format6 === "der")
    ...
    toHex(format6) {
      return bytesToHex2(this.toBytes(format6));
    }
    // TODO: remove
    assertValidity() {
    }
  ...
  function validateMsgAndHash(message, prehash) {
    ...
    return prehash ? _abytes2(hash7(message), void 0, "prehashed message") : message;
  }
  function prepSig(message, privateKey, opts) {
    ...
    if (extraEntropy2 != null && extraEntropy2 !== false) {
      const e = extraEntropy2 === true ? randomBytes4(lengths.secretKey) : extraEntropy2;
  ...
  function sign6(message, secretKey, opts = {}) {
    ...
    const drbg = createHmacDrbg(hash7.outputLen, Fn.BYTES, hmac4);
  ...
  function verify6(signature, message, publicKey, opts = {}) {
    const { lowS, prehash, format: format6 } = validateSigOpts(opts, defaultSigOpts);
    publicKey = ensureBytes2("publicKey", publicKey);
    message = validateMsgAndHash(ensureBytes2("message", message), prehash);
    if ("strict" in opts)
      throw new Error("options.strict was renamed to lowS");
    const sig = format6 === void 0 ? tryParsingSig(signature) : Signature.fromBytes(ensureBytes2("sig", signature), format6);
    if (sig === false)
      return false;
    try {
  ...
  return Object.freeze({
    keygen,
    getPublicKey: getPublicKey6,
    getSharedSecret: getSharedSecret4,
    utils: utils2,
    lengths,
    Point: Point2,
    sign: sign6,
...
function weierstrass(c) {
  const { CURVE, curveOpts, hash: hash7, ecdsaOpts } = _ecdsa_legacy_opts_to_new(c);
  const Point2 = weierstrassN(CURVE, curveOpts);
  const signs = ecdsa(Point2, hash7, ecdsaOpts);
  return _ecdsa_new_output_to_legacy(c, signs);
}

// node_modules/@scure/bip32/node_modules/@noble/curves/esm/_shortw_utils.js
function createCurve2(curveDef, defHash) {
  const create4 = (hash7) => weierstrass({ ...curveDef, hash: hash7 });
...
function hash_to_field(msg, count, options) {
  ...
  const { p, k, m, hash: hash7, expand, DST } = options;
...
function schnorrVerify(signature, message, publicKey) {
  ...
  try {
    ...
    if (!inRange(r, _1n11, secp256k1_CURVE.p))
      return false;
...
function join(separator = "") {
  ...
  return {
    encode: (from24) => {
      astrArr("join.decode", from24);
      return from24.join(separator);
    },
    decode: (to2) => {
...
function normalize(fn) {
  afn(fn);
  return { encode: (from24) => from24, decode: (to2) => fn(to2) };
}
function convertRadix(data, from24, to2) {
  if (from24 < 2)
    throw new Error(`convertRadix: invalid from=${from24}, base cannot be less than 2`);
  if (to2 < 2)
  ...
  while (true) {
    ...
    for (let i = pos; i < dlen; i++) {
      ...
      const fromCarry = from24 * carry;
      const digitBase = fromCarry + digit;
      if (!Number.isSafeInteger(digitBase) || fromCarry / from24 !== carry || digitBase - digit !== fromCarry) {
...
var radix2carry = (from24, to2) => from24 + (to2 - gcd(from24, to2));
var powers = (() => {
  let res = [];
  for (let i = 0; i < 40; i++)
    res.push(2 ** i);
...
function convertRadix2(data, from24, to2, padding2) {
  aArr(data);
  if (from24 <= 0 || from24 > 32)
    throw new Error(`convertRadix2: wrong from=${from24}`);
  if (to2 <= 0 || to2 > 32)
    throw new Error(`convertRadix2: wrong to=${to2}`);
  if (radix2carry(from24, to2) > 32) {
    throw new Error(`convertRadix2: carry overflow from=${from24} to=${to2} carryBits=${radix2carry(from24, to2)}`);
  }
  let carry = 0;
  let pos = 0;
  const max = powers[from24];
  const mask = powers[to2] - 1;
  const res = [];
  for (const n of data) {
    anumber3(n);
    if (n >= max)
      throw new Error(`convertRadix2: invalid data word=${n} from=${from24}`);
    carry = carry << from24 | n;
    if (pos + from24 > 32)
      throw new Error(`convertRadix2: carry overflow pos=${pos} from=${from24}`);
    pos += from24;
    for (; pos >= to2; pos -= to2)
...
function genBech32(encoding) {
  ...
  function encode7(prefix, words, limit2 = 90) {
...
function wrapConstructorWithKey(hashCons) {
  const hashC = (msg, key) => hashCons(key, msg.length).update(toBytes9(msg)).digest();
...
function toKey(keystore, options) {
  const { crypto: crypto4 } = keystore;
  const { password } = options;
  const { cipherparams, kdf, kdfparams } = crypto4;
...
function fromRpc5(log, _options = {}) {
  return {
    ...log,
...
var hmac3 = (hash7, key, message) => new HMAC2(hash7, key).update(message).digest();
hmac3.create = (hash7, key) => new HMAC2(hash7, key);

// node_modules/@scure/bip39/node_modules/@noble/hashes/esm/pbkdf2.js
function pbkdf2Init2(hash7, _password, _salt, _opts) {
  ahash3(hash7);
  ...
  const DK = new Uint8Array(dkLen);
  const PRF = hmac3.create(hash7, password);
  const PRFSalt = PRF._cloneInto().update(salt);
  return { c, dkLen, asyncTick, DK, PRF, PRFSalt };
}
...
function pbkdf23(hash7, password, salt, opts) {
  const { c, dkLen, DK, PRF, PRFSalt } = pbkdf2Init2(hash7, password, salt, opts);
  let prfW;
  const arr = new Uint8Array(4);
...
function mnemonicToSeedSync(mnemonic, passphrase = "") {
  return pbkdf23(sha5123, normalize2(mnemonic).nfkd, psalt(passphrase), { c: 2048, dkLen: 64 });
}
...
function validate6(mnemonic, wordlist11) {
...
function getSharedSecret(options) {
  ...
  const point = secp256r12.ProjectivePoint.fromHex(toHex2(publicKey).slice(2));
...
function verify3(options) {
  ...
  return secp256r12.verify(signature, payload instanceof Uint8Array ? payload : fromHex(payload), toHex2(publicKey).substring(2), ...hash7 ? [{ prehash: true, lowS: true }] : []);
}
...
function fromRpc6(rpcStateOverrides) {
...
function from18(value, decimals = 0) {
...
function fromEther(ether, unit = "wei") {
  return from18(ether, exponents.ether - exponents[unit]);
}
function fromGwei(gwei, unit = "wei") {
  return from18(gwei, exponents.gwei - exponents[unit]);
}
...
function fromRpc7(receipt) {
  ...
  return {
    ...
    logs: receipt.logs.map((log) => fromRpc5(log, { pending: false })),
  ...
  const validateData = (struct, data) => {
    for (const param of struct) {
      ...
      if (bytesMatch) {
        ...
        if (size4 && size2(value2) !== Number.parseInt(size4, 10))
...
function getSignPayload9(options) {
...
async function createKeyPair4(options = {}) {
...
function createKeyPair5(options = {}) {
  ...
  const publicKey = getPublicKey5({ privateKey, as });
  return {
    privateKey,
    publicKey
  };
}
function getPublicKey5(options) {
  const { as = "Hex", privateKey } = options;
  const privateKeyBytes = from(privateKey);
...
function getSharedSecret3(options) {
  const { as = "Hex", privateKey, publicKey } = options;
  const privateKeyBytes = from(privateKey);
  const publicKeyBytes = from(publicKey);
...
@noble/curves/esm/p256.js:
  (*! noble-curves - MIT License (c) 2022 Paul Miller (paulmillr.com) *)
*/
//# sourceMappingURL=ens-ZCBBJXD6.js.map
