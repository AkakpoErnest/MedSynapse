...
async function main() {
  console.log("🚀 Deploying MedSynapse Token Simple (gas optimized)...");
  
  // Get the contract factory
  const MedSynapseTokenSimple = await ethers.getContractFactory("MedSynapseTokenSimple");
  
  // Estimate gas first
  console.log("⛽ Estimating gas...");
  const estimatedGas = await MedSynapseTokenSimple.getDeployTransaction().then(tx => 
  ...
  const medSynapseToken = await MedSynapseTokenSimple.deploy({
    gasLimit: estimatedGas + ethers.parseUnits("50000", "wei"), // Smaller buffer
    gasPrice: gasPrice
  });
  ...
  console.log("✅ MedSynapse Token Simple deployed to:", tokenAddress);
